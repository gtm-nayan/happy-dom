 ...
 * XML parser.
 */
export default class XMLParser2 {
	/**
			...
			while ((match = markupRegexp.exec(data))) {
				...
				if (parent && match.index !== lastTextIndex) {
					...
					if (parentTagName && PlainTextElements.includes(parentTagName)) {
						...
						const conditionCommentRegExp = new RegExp(CONDITION_COMMENT_REGEXP, 'gi');
						const conditionCommentEndRegExp = new RegExp(CONDITION_COMMENT_END_REGEXP, 'gi');
						// @Refer: https://learn.microsoft.com/en-us/previous-versions/windows/internet-explorer/ie-developer/?redirectedfrom=MSDN
						if (
							isStartTag &&
							(condCommMatch = conditionCommentRegExp.exec(text)) &&
							condCommMatch[0] &&
							(condCommEndMatch = conditionCommentEndRegExp.exec(
								data.substring(markupRegexp.lastIndex)
							)) &&
							condCommEndMatch[0]
						) {
							markupRegexp.lastIndex += conditionCommentEndRegExp.lastIndex;
							continue;
						} else {
